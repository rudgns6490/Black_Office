...
function filtering(event) {
  ...
  var fillter = $('input:checkbox.filter:checked').map(function () {  // 필터링 되는 체크박스  값 
    return $(this).val();
  }).get();
  ...
  show_username = fillter.indexOf(event.departmentname) >= 0;  // 필터 되는곳 departname 
  ...
  events: 
	  function (start, end, timezone, callback) {
		  ...
		  $.ajax({
		      type: "get",
		      dataType : "json", 
		      url: "/controller/departmentScheduleJSONList.action",
		      data: {
