...
public class BoardService implements InterBoardService {
	...
	// === #80. 1개글 삭제하기(댓글쓰기가 없는 게시판) === //
		...
		public int del(BoardVO boardvo) {
			...
			if(!bool) { // 로그인 아이디가 작성자 아이디와 일치하지 않는 경우
				...
				if( commentCount > 0 ) {
					...
					m = dao.deleteComent(boardvo.getSeq());
				}
				// ▲ 2020.01.14.화
				/////////////////////////////////////////////////////////////////
				
				int n = dao.deleteBoard(boardvo.getSeq());
				// return n;
		...
		public int addComment(CommentVO commentvo) throws Throwable {
			...
			if( n == 1 ) {
				result = dao.updateCommentCount(commentvo.getParentSeq()); // 원게시물tblBoard 테이블에 commentCount 컬럼의 값을 1증가(update)
			}
